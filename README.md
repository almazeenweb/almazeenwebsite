# AL Mazen Gutter LTD — Website

A static, no-build website for AL Mazen Gutter LTD (gutter, siding, roofing, and soffit &
fascia services across Alberta, based in Calgary), built to be hosted on GitHub Pages.
...
resources/           Logo/favicon icon, and image assets
  icon.ico           Logo mark + browser tab icon
...
2. **Custom domain on GitHub Pages** (optional): if using a custom domain, add a `CNAME`
   file at the repo root containing just the domain (e.g. `almazengutter.ca`), then
